...
struct Maze * solveMaze (struct Maze maze, int * isThereAns){
    ...
    for(int k = 0; k < pS; k++){ // Visual representation
        ...
        if (ans->matrix[changing.i][changing.j] == ' ')
            ans->matrix[changing.i][changing.j] = '+';
    }
    ...
    char * mGallery (int pos) { // Gallery of saved ASCII mazes
        if (pos == 0) {
            return "s ###################\n"
                   "      # # #       # #\n"
                   "# ### # # # # # ### #\n"
...
int main() {
    ...
    printf("Hello!, welcome to lab_Solver, made by @guillerubio\n");
    usleep(1000000);
    printf("Press: -> '0' to solve a random prefabricated ASCII maze,\n       -> '1' to solve your own maze,"
           "\n       -> any other character to stop the program\n");
    char ans0 = ' ';
    scanf("%c", &ans0);
    if (ans0 == '0') {
        printf("Great, this is your randomly selected maze:\n");
        srand(time(NULL));
        int random_number = rand() % 5;
        input = mGallery(random_number);
    } else if (ans0 == '1') {
        printf("Great, let's solve your maze, \n");
        printf("Remember that to edit your own maze you should insert it in the first line of the main\n");
        printf("following the example given.\n");
    } else {
        printf("Goodbye!\n");
        return 0;
    }
    ...
    struct Maze solvedMaze = *solveMaze(*maze, &isThereAns);
    if (isThereAns) {
        ...
        usleep(3000000);
        printf("\n");
        printf("-----OUTPUT MAZE----- \n%s\n", mazeToString(solvedMaze));
    } else {
        printf("The algorithm has detected that this maze does not have an answer\n");
    }
    free(maze);
    usleep(1500000);
    printf("Goodbye\n");
